...
***** This file should be named 35688.txt or 35688.zip *****
This and all associated files of various formats will be found in:
        http://www.gutenberg.org/3/5/6/8/35688/
...
def alice():
    """
    counts all d words in alice in wonderland
    """
    remove_punct(alice_in_wonderland)
    s = new_alice.lower()
    words = s.split()
    ...
    for word in words:   
        new_text[word] = new_text.get(word, 0) + 1
    ...
    for (item, count) in items:
        if RepresentsInt(item) == False and sec_head(item) != True:
            print(layout.format(item, count))
